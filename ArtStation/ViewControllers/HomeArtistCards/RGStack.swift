...
public enum CardMachine {
    public enum Position {
        case back, front, bottom, off,bottom2,bottom3
    }
...
public struct RGStack<CardView: ConfigurableCard>: View {
    ...
    let index0Positions: [CardMachine.Position] = [.front, .bottom, .bottom2,.bottom3,.off, .off]
    let index1Positions: [CardMachine.Position] = [.back, .front, .bottom,.bottom2, .bottom3,.off]
    ...
    @State private var positions: (last: [CardMachine.Position], current: [CardMachine.Position]) =
        ([.front, .bottom,.bottom2,.bottom3, .off, .off], [.front, .bottom,.bottom2,.bottom3,.off, .off])
    ...
    public var body: some View {
        ...
        .frame(width: stackSize(style: cardInfo).width, height: stackSize(style: cardInfo).height, alignment: .center)
        
    
        .on(dragged: { value in
            self.set(drag: self.variance(from: value.translation))
        }, ended: { value in
            print(value.translation,self.snapThresholdMangnitude)
...
public extension RGStack {
    ...
    private func tapGesture() {
        reconfigurePostInteraction(progress: 0.0, direction: .touch, threshold: 0.0)
    }
...
class testTapGestureForwading:ObservableObject
{
    var dataCheck="false"
}
