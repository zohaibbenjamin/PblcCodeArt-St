...
class EntertainerPackageViewController:  UIViewController, UIScrollViewDelegate {
    
    @IBOutlet weak var scrollview: UIScrollView!
    
    //@IBOutlet weak var ratingView: CosmosView!
    //MARK: - IB Refrences
    @IBOutlet weak var specialitiesCollectionViewEntertainer:UICollectionView!
    ...
    var specialitiesDataSource: [String]?{
        didSet{
            self.specialitiesCollectionViewEntertainer.dataSource = self
            self.specialitiesCollectionViewEntertainer.delegate = self
            self.specialitiesCollectionViewEntertainer.contentInsetAdjustmentBehavior = .always
            self.specialitiesCollectionViewEntertainer.reloadData()
            
            
            let specialitiesCollectionHeight = specialitiesCollectionViewEntertainer.collectionViewLayout.collectionViewContentSize.height
    ...
    @IBAction func createCustomPackage(_ sender:UIButton){
        if DataManager.getUserLog == "guest"
        {
            ...
            let viewController = Utils.getViewController(storyboard: StoryboardId.artistPackagesStoryboard, identifier: StoryboardId.artistPackagesCreatCustomPackageViewControllerId, viewController: CreateCustomPackageViewController.self)
    ...
    @IBOutlet weak var entertainerPackageDetail:UILabel!
    
    var artistId : String?
    var imagesArray : [String] = []
    var videosArray : [String] = []
    var photoIndex:Int = 0
    var totalEventPrice:Double = 0.0
...
//        }
        ...
        //        guard self.artistTalent?.speciality != nil else {
        //            return
        //        }
...
//            }
            ...
            {
                self.showAlert(title: "", message: Utils.localizedText(text: "please select play time"))
                return
            }
    ...
    func claimBooking() {
        self.viewModel.cliamBooking(param: ["artist_id":self.artistId ?? 0,"package_id":self.packageInfo?.id ?? 0]) { isSuccess, messageError in
            if isSuccess{
                ...
                debugPrint(customBookingV2.sharedInstance.userType)
                
                customBookingV2.sharedInstance.requested_place = DataManager.requestPlaceToBeSent
                customBookingV2.sharedInstance.artist_id = self.artistInfo?.userID ?? 0
               // customBookingV2.sharedInstance.package_id = self.packageInfo?.id ?? 0
                ...
                if rangeType.elementsEqual("2"){
                    ...
                    customBookingV2.sharedInstance.playTime = Int(self.packageInfo?.playTime ?? 0)
                }
    ...
    func loadPackageData(){
        ...
        if DataManager.getLanguage == Language.english.rawValue{
            self.entertainerPackageDetail.text = packageInfo?.offer ?? ""
            ...
            self.entertainerPackageDetail.text = packageInfo?.offer_ar ?? ""
            let eventprice = self.packageInfo!.eventPrice ?? 0
...
extension EntertainerPackageViewController: UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        }else if collectionView == specialitiesCollectionViewEntertainer {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        ...
        if collectionView == specialitiesCollectionViewEntertainer{
